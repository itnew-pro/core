...
/**
 * Файл класса m140124_000000_staff.
 *
 * Миграция для сотрудников
 *
...
class m140124_000000_staff extends CDbMigration
{
	...
	public function safeUp()
	{
		...
		if (!Yii::app()->db->schema->getTable("staff_group")) {
			...
			$this->addForeignKey("staff_group_staff_id", "staff_group", "staff_id", "staff", "id");
		}
		...
		if (!Yii::app()->db->schema->getTable("staff_content")) {
			...
			$this->addForeignKey("staff_content_seo_id", "staff_content", "seo_id", "seo", "id");
			$this->addForeignKey("staff_content_photo", "staff_content", "photo", "images", "id");
			$this->addForeignKey("staff_content_description", "staff_content", "description", "text", "id");
			$this->addForeignKey("staff_content_text", "staff_content", "text", "text", "id");
			$this->addForeignKey("staff_content_group_id", "staff_content", "group_id", "staff_group", "id");
			$this->createIndex("staff_content_sort", "staff_content", "sort");
		}
	}
